...
<template>
  <div class="ps-3 pe-3">
    <ul class="nav nav-tabs" :id="`tab-${videoId}`" role="tablist">
      ...
      <li class="nav-item" role="presentation" v-for="(game, idx) in videoDetails.videoGame" :key="idx">
    ...
    <div class="tab-content" :id="`tab-content-${videoId}`">
      ...
      <div class="tab-pane fade" :id="`games-${videoId}-${idx}-tab-pane`" role="tabpanel" aria-labelledby="games-tab"
           :tabindex="idx+1" v-for="(game, idx) in videoDetails.videoGame" :key="idx">
        <div class="row">
          <div class="col">
            <table class="table table-bordered">
              <tbody>
                ...
                <tr>
                  <th>Openings</th>
                  <td>
                    <span v-for="opening in getOpeningsForGame(idx)" :key="opening" style="display: block; background: inherit;">
                      {{ opening }}
                    </span>
                  </td>
                </tr>
